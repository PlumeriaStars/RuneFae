...
public class ShadowEnemy : MonoBehaviour
{
    ...
    [SerializeField] FloatingHealthBarS _enemyHealth;
    ...
    [SerializeField] GameObject _enemyProjectileRight = null;
    [SerializeField] GameObject _enemyProjectileLeft = null;
    ...
    {
        if (Vector2.Distance(player.position, transform.position) > 2f && Vector2.Distance(_currentPatrolPoint.position, transform.position) < 0.5f)
            Move();
    ...
    private void CheckAttackCooldown()
    {
        ...
        if (_attackTimer <= 0)
        {
            _animatorController.SetTrigger("shadowAttack");
    ...
    public void ShadowAttack()
    {
        ...
        if (!_spriteRender.flipX)
            Instantiate(_enemyProjectileLeft, transform.position + new Vector3(-0.5f, 0f, 0f), Quaternion.Euler(0f, 0f, 0f));
        else
            Instantiate(_enemyProjectileRight, transform.position + new Vector3(0.5f, 0f, 0f), Quaternion.Euler(0f, 0f, 0f));
    }
