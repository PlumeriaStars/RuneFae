...
public class ShrineControllerTier3 : MonoBehaviour
{
    [Header("UI Elements")]
    ...
    [SerializeField] ShrineListTier3 _shrineListController = null;
    ...
    private void RollItem(int itemNum)
    {
        ...
        {
            _shrineListController.SetHealth03(itemNum);
            SetSprite(itemNum, 0);
        }
        else if (randomValue > healthBound && randomValue <= speedBound)
        {
            _shrineListController.SetSpeed03(itemNum);
            SetSprite(itemNum, 1);
        }
