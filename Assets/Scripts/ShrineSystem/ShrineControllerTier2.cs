...
public class ShrineControllerTier2 : MonoBehaviour
{
    [Header("UI Elements")]
    ...
    [SerializeField] ShrineListTier2 _shrineListController = null;
    ...
    private void RollItem(int itemNum)
    {
        ...
        {
            _shrineListController.SetHealth02(itemNum);
            SetSprite(itemNum, 0);
        }
        else if (randomValue > healthBound && randomValue <= speedBound)
        {
            _shrineListController.SetSpeed02(itemNum);
            SetSprite(itemNum, 1);
        }
